...
public class Block {
    /**
     * 2. 区块头中存储的 POH 信息
     * 区块头不会存储每笔交易的完整 POH 记录，而是存储整个区块在 POH 链中的 “全局锚点”，主要包括：
     * 区块起始时的 POH 哈希：即区块中第一笔交易的previousHash，用于关联上一个区块的 POH 链；
     * 区块结束时的 POH 哈希：即区块中最后一笔交易的currentHash（或区块结束时的 Tick 哈希），作为本区块 POH 链的终点；
     * 区块覆盖的 Tick 范围：比如区块包含的起始 Tick 和结束 Tick，用于快速定位区块在全局 POH 时间线中的位置。
     * 这些信息的作用是：让其他节点无需遍历区块内所有交易，就能快速验证该区块是否嵌入全局 POH 链，以及区块的时间范围。
     */
